...
A Python tool for discovering patterns in blood test results using unsupervised machine learning. Upload a dataset of real patient results and the app identifies natural clusters in each marker — without pre-labelling them — so you can interpret what those clusters mean.
...
3. **Cluster Explorer** shows the discovered clusters for each marker — their means, ranges, and patient counts — alongside reference range lines for context
4. **Patient View** shows each patient's cluster assignments and flags markers where they fall in the smallest (most unusual) cluster
...
**Unit auto-detection:** Six markers automatically detect and convert between unit systems:
...
All 27 markers have male reference ranges defined in `thresholds.py`. These are used in:
- The **Reference Lookup** tab (single value check)
- The **Cluster Explorer** histogram (shown as dotted lines for context, not used to label clusters)
...
```
app.py              — Streamlit UI (3 tabs: Reference Lookup, Upload & Discover, Patient View)
...
unit_conversions.py — Auto-detects and converts between unit systems
tests/              — 159 tests across all modules
```
...
- Persist uploaded data to SQLite for longitudinal analysis
- Retrain clusters as more data arrives (online learning)
